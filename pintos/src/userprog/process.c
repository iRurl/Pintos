...
void set_exit_status(struct thread* t, int status) {
...
static char* get_argv0(const char* file_name_) {
...
bool syscall_lock_acquire(char* lock) {
  ...
  if (node == NULL || lock_held_by_current_thread(&node->lock)) return false;
...
void process_exit(void) {
  ...
  if (pcb == NULL) {
    if (node != NULL)  sema_up(&node->semaph);
    thread_exit();
    NOT_REACHED();
  }
  ...
  if (t->pcb != NULL && t->pcb->pagedir != NULL) {
    ...
    if (kpage != NULL) {
      ...
      for (int i = 0; i < max_pthread_num; i++) {
        base -= PGSIZE;
        if (!pagedir_is_accessed(t->pcb->pagedir, (uint8_t*)base - PGSIZE)) break;
      }
      if (t->pcb->pagedir != NULL)
...
void pthread_exit_main(void) {
  ...
  sema_up(&p->semaph);
  while (true) {
    if (list_empty(&p->all_threads))  break;
